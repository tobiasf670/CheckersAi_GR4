package GUI;
import BoardLogic.CheckerPosition;
import Controller.CheckersController;
import Enums.PlayerSide;
...
public class GUIBoard extends JComponent
{
    private boolean isInvalidPlayerMove;
   CheckerPosition start;
   CheckerPosition goal;
   ...
   // preferred size of BoardLogic component
   ...
   public GUIBoard(CheckersController controller)
   {
      ...
      addMouseListener(new MouseAdapter()
                       {
                          ...
                          public void mousePressed(MouseEvent me)
                          {
                             ...
                             int boardX = x / SQUAREDIM;
                             int boardY = y / SQUAREDIM;

                              start = new CheckerPosition(boardX,boardY);
                             ...
                             for (PosCheck posCheck: posChecks)
                                if (Checker.contains(x, y, posCheck.cx, 
                                                     posCheck.cy))
                                {
                                   GUIBoard.this.posCheck = posCheck;
                                   oldcx = posCheck.cx;
                                   oldcy = posCheck.cy;
                          ...
                          public void mouseReleased(MouseEvent me)
                          {
                             ...
                             int y = me.getY();
                              int boardX = x / SQUAREDIM;
                              int boardY = y / SQUAREDIM;
                             goal = new CheckerPosition(boardX,boardY);
                             //boolean isValidMove = isValidMove(HUMAN,start,goal);
                             boolean isValidMove = controller.getBoard().isValidMove(start, goal, PlayerSide.RED);
                             ...
                             for (PosCheck posCheck: posChecks)
                                if (posCheck != GUIBoard.this.posCheck &&
                                    posCheck.cx == GUIBoard.this.posCheck.cx &&
                                    posCheck.cy == GUIBoard.this.posCheck.cy || !isValidMove)
                                {
                                   GUIBoard.this.posCheck.cx = oldcx;
                                   GUIBoard.this.posCheck.cy = oldcy;
                                   isInvalidPlayerMove = true;
                                }
                             posCheck = null;
                             repaint();
                             if(!isInvalidPlayerMove) {
                                 controller.movePerformed(start, goal);
                             }
                          }
                       });
